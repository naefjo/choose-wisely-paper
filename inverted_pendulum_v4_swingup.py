...
class InvertedPendulumSwingupEnv(MujocoEnv, utils.EzPickle):
    """
    ## Description
    ...
    * v0: Initial versions release (1.0.0)
    ...
    def __init__(self, **kwargs):
        ...
        xml_filename = path.join(
            path.dirname(__file__), "inverted_pendulum_swingup.xml"
        )
        MujocoEnv.__init__(
            self,
            xml_filename,
    ...
    def step(self, a):
        ...
        ob = self._get_obs()
        terminated = bool(
            not np.isfinite(ob).all() or (np.abs(ob[1]) > 7)
        )  # or (np.abs(ob[1]) > 1.5)
    ...
    def reset_model(self):
        ...
        qvel = self.init_qvel + self.np_random.uniform(
            size=self.model.nv, low=0.001, high=0.02
        )
